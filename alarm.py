...
def alarmclock(alarmtime):
    while True:
        ...
        if time_now == alarmtime:
            ...
            print("wake up!")
            mixer.init()
            mixer.music.load(r'C:\Users\greeshma\Desktop\mp3_elektro.mp3')
            mixer.music.play()
            break
def alarm_off():
    if(a==1):
...
setbtn = Button(root, text="set alarm", command=setalarm, bg="DodgerBlue2",fg="white", font=('arial', 20, 'bold')).grid(row=4, column=2)
stopbtn = Button(root, text="OFF", command=alarm_off(), bg="DodgerBlue2",fg="white", font=('arial', 20, 'bold')).grid(row=4, column=3)
